...
export default function ScoreUpdate()
{
  ...
  const newScore= (a,b) =>{
    fetch("http://54.243.238.129:8082/updateScores?teama="+a+"&teamb="+b+"&matchid="+m.match_id)
    .then(resp => resp.json())
    .then(obj => {
  ...
  const winTeam= (teamid) =>{
    fetch("http://54.243.238.129:8082/changeTeamMatchStatusWin?teamid="+teamid)
    .then(resp => resp.json())
    .then(obj => {
      console.log(JSON.stringify(obj))
      if (obj===1) {
  ...
  const looseTeam= (teamid) =>{
    fetch("http://54.243.238.129:8082/changeTeamMatchStatusLoose?teamid="+teamid)
    .then(resp => resp.json())
    .then(obj => {
      console.log(JSON.stringify(obj))
      if (obj===1) {
